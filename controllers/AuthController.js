...
export const register = async (req, res) => {
    ...
    try {
        const user = await User.findOne({nickname: nickname})
        if(user) {
            return res.status(400).json({success: false, message: 'Tên đăng nhập đã tồn tại, vui lòng thử tên đăng nhập khác!'})
        } 
        // All good
        const hashedPassword = await argon2.hash(password)
        const newUser = new User({ nickname: nickname, password: hashedPassword, fullname: fullname })
        await newUser.save()
